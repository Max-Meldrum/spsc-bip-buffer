...
/// Creates a new `BipBufferWriter`/`BipBufferReader` pair using the provided underlying storage
pub fn bip_buffer_from_vec<T>(vec: Vec<T>) -> (BipBufferWriter<T>, BipBufferReader<T>)
where
    T: Copy + 'static,
{
    bip_buffer_from(vec.into_boxed_slice())
}

impl<T> BipBuffer<T> {
    // NOTE: Panics if B is not the type of the underlying storage.
    fn into_inner<B: std::ops::DerefMut<Target = [T]> + 'static>(self) -> B {
...
impl<T> BipBufferWriter<T> {
    ...
    pub fn try_unwrap<B: std::ops::DerefMut<Target = [T]> + 'static>(self) -> Result<B, Self> {
        let BipBufferWriter {
            buffer,
            write,
            last,
...
mod tests {
    ...
    fn spsc_with_struct() {
        ...
        use crate::bip_buffer_from_vec;
        use std::mem::{self, MaybeUninit};
        let data: [MaybeUninit<Tester>; 128] = unsafe { MaybeUninit::uninit().assume_init() };
        let my_data = unsafe { mem::transmute::<_, [Tester; 128]>(data) };
        let (mut writer, mut reader) = bip_buffer_from_vec(my_data.to_vec());
        let sender = std::thread::spawn(move || {
            for _ in 0..200 {
                let t1 = Tester { x: 16, u: 24 };
                let t2 = Tester { x: 10, u: 2 };
                let t3 = Tester { x: 6, u: 15 };
                let t4 = Tester { x: 1, u: 4 };
                writer.spin_reserve(4).copy_from_slice(&[t1, t2, t3, t4]);
            }
        });
        let receiver = std::thread::spawn(move || {
            let t1 = Tester { x: 16, u: 24 };
            ...
            let t4 = Tester { x: 1, u: 4 };
            for _ in 0..200 {
                while reader.valid().len() < 4 {}
                assert_eq!(&reader.valid()[..4], &[t1, t2, t3, t4]);
                reader.consume(4);
            }
        });
        sender.join().unwrap();
